...
const DashBoard = async () => {
  ...
  const getData = async () => {
    if (user) {
      const res = await fetch(`http://assignment-9-backend-k687.vercel.app/user/${user.email}`);
      const reqes = await fetch ('http://assignment-9-backend-k687.vercel.app/pet/req',{
        headers:{
